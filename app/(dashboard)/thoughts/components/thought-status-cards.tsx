  ...
  return (
    <div className="flex gap-2 sm:gap-3 mb-4 sm:mb-6 flex-wrap overflow-x-auto pb-2 -mx-1 px-1">
      ...
      >
        ...
        <span className="font-semibold text-white text-sm sm:text-base">{stats.total}</span>
      ...
      {statusConfigs.map(config => {
        ...
        return (
          ...
          >
            <Icon size={16} strokeWidth={1.5} style={{ color: config.color }} className="sm:w-[18px] sm:h-[18px]" />
            <span className="font-semibold text-white text-sm sm:text-base">{count}</span>
            <span className="text-xs sm:text-sm text-white/60 whitespace-nowrap">{t(config.labelKey, locale)}</span>
          </Link>
        )
      })}
    </div>
